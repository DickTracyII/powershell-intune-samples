...
try {
    ...
    if($ExcludeMDM.IsPresent){ $Count_Params++ }
        ...
        $uri = "$global:GraphEndpoint/$graphApiVersion/$Resource`?`$filter=managementAgent eq 'mdm' and managementAgent eq 'easmdm' and managementAgent eq 'googleCloudDevicePolicyController'"
...
if($Devices){
    ...
    $uri = "beta/deviceManagement/manageddevices('$DeviceID')?`$select=hardwareinformation,iccid,udid,ethernetMacAddress"

    $DeviceInfo = (Invoke-IntuneRestMethod -Uri $uri -Method GET)
